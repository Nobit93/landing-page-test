...
const Header: React.FC = () => {
  ...
  const headerScrolledClasses = "bg-[rgba(255,255,255,0.8)] backdrop-blur-[44px] border-b border-[rgba(0,0,0,0.08)]";
    ...
    <header className={`${headerBaseClasses} ${isScrolled ? headerScrolledClasses : 'bg-transparent border-transparent'}`}>
      <nav className="flex items-center justify-between gap-10 px-10 py-2.5 max-w-screen-xl mx-auto">
        ...
        <div className="flex items-center gap-10">
          <Link href="/">
            <div className="flex items-center gap-2.5 hover:opacity-80 transition-opacity">
              <div className={`w-8 h-8 ${isScrolled ? 'bg-blue-600' : 'bg-white'} rounded-full`}></div>
              <span className={`text-xl font-bold ${logoTextColorClass}`}>X-Design</span>
              <span className={`text-sm ${logoSubTextColorClass}`}>(Vmake AI)</span>
            </div>
          </Link>
          <div className="hidden md:flex items-center gap-1">
            {navItems.map((item) => (
              ...
              >
                ...
                {item.hasDropdown && (
                  <svg width="18" height="10" viewBox="0 0 18 10" fill="none" xmlns="http://www.w3.org/2000/svg" className={`w-4 h-4 ${textColorClass}`}>
        ...
        <div className="flex items-center gap-4">
           <Link href="#" className={`px-3 h-10 flex items-center rounded-lg ${isScrolled ? 'hover:bg-gray-100' : 'hover:bg-[rgba(255,255,255,0.1)]'} transition-colors duration-200`}>
              <span className={`text-sm font-semibold ${textColorClass}`}>Top up</span>
           </Link>
          <button className={`px-3 h-10 ${isScrolled ? 'bg-[#0A68DA] hover:bg-[#0854AE]' : 'bg-white hover:bg-gray-200 text-[#0A68DA]'} ${isScrolled ? 'text-white' : 'text-[#0A68DA] border border-transparent'} text-[13px] font-semibold leading-[1.21] rounded-lg transition-colors duration-200`}>
            Get Started
          </button>
        </div>
      </nav>
